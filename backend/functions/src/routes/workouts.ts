...
workoutsRouter.get("/weekly", requireAuth, async (req: AuthedRequest, res) => {
...
workoutsRouter.get("/today", requireAuth, async (req: AuthedRequest, res) => {
  const snapshot = await db()
    .collection("workout_plans")
    .where("user_id", "==", req.user!.uid)
    .orderBy("created_at", "desc")
...
workoutsRouter.post("/complete", requireAuth, async (req: AuthedRequest, res) => {
