...
gamificationRouter.get("/xp", requireAuth, async (req, res) => {
    const xp = await getXp(req.user.uid);
    res.json(xp);
});
gamificationRouter.get("/streak", requireAuth, async (req, res) => {
    const xp = await getXp(req.user.uid);
    res.json({ streak_count: xp.streak_count, last_active_date: xp.last_active_date });
});
gamificationRouter.get("/achievements", requireAuth, async (_req, res) => {
    res.json([
